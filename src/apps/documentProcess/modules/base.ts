...
export const removeDiacritics = (text: string) => text.replaceAll(/(ّ|َ|ِ|ُ|ً|ٍ|ٌ|ْ|ء)/g, '').replaceAll(/(إ|أ)/g, 'ا').replaceAll(/(ي|ئ)/g, 'ی').replaceAll(/ؤ/g, 'و').replaceAll(/ك/g, 'ک').replaceAll(/(ۀ|ة|هٔ)/g, 'ه');
